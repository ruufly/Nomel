...
table = ttk.Treeview(tableframe,height=12,columns=columns,show='headings',)
    ...
    for name in files:
        if os.path.splitext(name)[-1] == ".nfctmp":
            ...
            try:
                ...
                inserttable(contest.name,contest.rule,len(contest.problems),usefulfuncs.runtimee(contest.starttime),usefulfuncs.runtimee(contest.endtime))
            except Exception:
                messagebox.showerror('Nomel','无法正确地获取本地保存的文件')
...
def openfile(*args):
    filename = filedialog.askopenfile(title='打开比赛文件',filetypes=[("比赛文件","*.nfct"),("All files","*")])
    if (filename == ''): return
    f = open("lastcontest\\newlast.dat","wb")
...
def newcon(*args):
    ...
    def mustnew(*args):
        aanewcon = createcontest.Contest(nameen.get(),
                                         createcontest.settime(stimeyearspin.get(),stimemonthspin.get(),stimedayspin.get(),
                                                               westimeyearspin.get(),westimemonthspin.get(),westimedayspin.get()),
                                         createcontest.settime(estimeyearspin.get(),estimemonthspin.get(),estimedayspin.get(),
                                                               wwestimeyearspin.get(),wwestimemonthspin.get(),wwestimedayspin.get()),
                                            varrul.get())
        aanewcon.save("lastcontest\\%s.tmp" % nameen.get())
        f = open("lastcontest\\newlast.dat","wb")
        pickle.dump("lastcontest\\%s.tmp" % nameen.get(),f)
        f.close()
        eg = "lastcontest\\" + nameen.get() + '.tmp'
        new.destroy()
        import contestedit
...
copyrightlabel = Label(root,text="Copyright 2023 distjr_",font=("Source Han Sans SC",8))
