...
try:
    ...
    messagebox.showerror('Nomel','FileNotFoundError: '+e,parent=root)
    exit()
...
try:
    ...
    messagebox.showerror('Nomel','文件已被移除或文件格式不受支持',parent=root)
    exit()

normallabel = Label(root,text="编辑您的比赛：%s" % contest.name,font=("Source Han Sans SC",20))

informationlabel = Label(root,text="文件基本信息：",font=("Source Han Sans SC",14))
informationslabel = Label(root,text="比赛名：%s  赛制：%s  开始时间：%s  结束时间：%s" % (contest.name,contest.rule,usefulfuncs.runtimee(contest.starttime),usefulfuncs.runtimee(contest.endtime)),font=("Source Han Sans SC",10))
...
def change(*args):
    ...
    def gomust(*args):
        ...
        contest.starttime = createcontest.settime(stimeyearspin.get(),stimemonthspin.get(),stimedayspin.get(),
                                                  westimeyearspin.get(),westimemonthspin.get(),westimedayspin.get())
        contest.endtime = createcontest.settime(estimeyearspin.get(),estimemonthspin.get(),estimedayspin.get(),
                                                wwestimeyearspin.get(),wwestimemonthspin.get(),wwestimedayspin.get())
        informationslabel.config(text="比赛名：%s  赛制：%s  开始时间：%s  结束时间：%s" % (contest.name,contest.rule,usefulfuncs.runtimee(contest.starttime),usefulfuncs.runtimee(contest.endtime)))
        root.update()
        new.destroy()
...
def newdata(isChange):
    ...
    contenlab = Label(nredatatk,text="注意：交互题请将程序源代码放在“输入数据”中，“输出数据”留空；\n   SPJ请将程序源代码放在“输出数据”中\n   提交答案题请将答案放在“输出数据”中，“输入数据”留空。")
    contenlab.place(x=5,y=270)

    def okbuttongo(*args):
        if not ecodeent.get():
            messagebox.showerror('Nomel','题目编号未填写',parent=root)
            return
        try:
            tl = float(tmeecodeent.get())
        except ValueError:
            messagebox.showerror('Nomel','时间限制未填写或不符合格式',parent=root)
            return
        try:
            ml = float(memecodeent.get())
        except ValueError:
            messagebox.showerror('Nomel','空间限制未填写或不符合格式',parent=root)
            return
        thisdatas[ecodeent.get()] = {'data':cincin.get('0.0',END),'timelimit':tl,'memorylimit':ml,'ans':ccincin.get('0.0',END),'score':kecodeent.get()}
        if isChange:
            mtable.delete(ccoder)
        mtable.insert('',END,values=[ecodeent.get(),thisdatas[ecodeent.get()]['timelimit'],thisdatas[ecodeent.get()]['memorylimit'],thisdatas[ecodeent.get()]['score']])
        nredatatk.destroy()
...
def newproblem(go):
    global thisdatas
    if go or messagebox.askyesno('Nomel','如果您现在正在编辑一个题目并没有保存的话，\n打开该题目会导致您的更改被舍弃。\n您真的要新建题目吗？',parent=root):
        thisdatas = {}
...
def openproblem(*args):
    global thisdatas
    if table.focus() and messagebox.askyesno('Nomel','如果您现在正在编辑一个题目并没有保存的话，\n打开该题目会导致您的更改被舍弃。\n您真的要打开题目吗？',parent=root):
        ...
        for i in contest.problems[nowpro].data:
            mtable.insert('',END,values=[i,contest.problems[nowpro].data[i]['timelimit'],contest.problems[nowpro].data[i]['memorylimit'],contest.problems[nowpro].data[i]['score']])
        thisdatas = contest.problems[nowpro].data

def delproblem(*args):
    if table.focus() and messagebox.askyesno('Nomel','您真的要删除该题目吗？\n该操作无法撤销！',parent=root):
